...
use ic_cdk::api::management_canister::ecdsa::{
	ecdsa_public_key,
	sign_with_ecdsa as ic_sign_with_ecdsa,
	EcdsaCurve,
	EcdsaKeyId,
...
pub async fn get_ecdsa_public_key(ctx: &BitcoinContext, derivation_path: Vec<Vec<u8>>) -> Vec<u8> {
	...
	let (public_key_resp,) = ecdsa_public_key(EcdsaPublicKeyArgument {
		canister_id: None,
	...
	let public_key = public_key_resp.public_key;
...
pub async fn sign_with_ecdsa(
	...
	let (sig_resp,) = ic_sign_with_ecdsa(SignWithEcdsaArgument {
		message_hash,
		derivation_path,
		key_id: EcdsaKeyId {
			curve: EcdsaCurve::Secp256k1,
	...
	Signature::from_compact(&sig_resp.signature).unwrap()
}
